...
$result_set=mysqli_query($conn,$query);
            ...
            echo "<tr class='gradeX'>
                  <td>$row->name</br>$row->email_address</td>
                  <td>$row->date_joined</td>  
                  <td>$row->expected_bal</br>$row->account_bal</td>  
                  <td>
           ...
           echo "<tr class='gradeX'>
                  <td>$row->name</br>$row->email_address</td>
                  <td>$row->date_joined</td> 
                  <td>shares:$row->expected_bal</br>wallet:$row->account_bal</td>
                  <td>
                  <form action='Controller/basicController.php' method='POST'>
                  <input type='hidden' name='user_id' value='$row->user_id'>
                 <button class='btn btn-success btn-xs' name='make_admin'>Make Admin</button>
                  <button class='btn btn-warning btn-xs' name='block_user'>unBlock User</button>
            ...
            if($row->user_status=='1'){
             ...
             echo "<tr class='gradeX'>
                  ...
                  <form action='Controller/basicController.php' method='POST'>
                  <input type='hidden' name='user_id' value='$row->user_id'>
                 <button class='btn btn-success btn-xs' name='remove_admin'>remove Admin</button>
